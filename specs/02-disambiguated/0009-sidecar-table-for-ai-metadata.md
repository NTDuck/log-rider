...
The initial plan proposed that after a log is written to the database with a `null` tag field, an asynchronous AI background job would query for unclassified logs, perform inference, and then execute an `UPDATE` query to attach the new tags.
...
The AI Consumer will consume from the Redpanda stream, perform its inference, and write tags strictly as new `INSERT` statements into this sidecar table. We explicitly prohibit any `UPDATE` queries on the primary log table.
